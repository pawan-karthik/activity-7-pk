function scatter_plot(data,
                      ...
                      colorCol = "",
                      margin = 50,
                      syncLegendState = null // Added shared synchronization object
) {
    ...
    function brushStart() { //// completed Missing part 4
        d3.selectAll("circle").classed("selected", false);
    }

    function brushed(event) {
        const selected_coordinates = d3.brushSelection(this); // Get brush selection bounds
        ...
        const inactiveCategories = new Set(
            Object.keys(syncLegendState).filter(category => !syncLegendState[category])
        );

        // Select elements within the brush area and filter out deselected categories
        d3.selectAll("circle").classed("selected", d => {
            ...
            const isActive = !inactiveCategories.has(d[colorCol]);
            return inBounds && isActive;
        });

        // Update the list to show only selected and active points
        ...
        listBox
            ...
            .text(
                d =>
                    `Model: ${d.Model}, Reliability: ${d.Reliability}, Type: ${d.Type}`
            );
    }
    ...
    legends_items.on("click", function (event, d) {
        const isActive = syncLegendState[d];
        syncLegendState[d] = !isActive;

        // Update visibility of points globally
        d3.selectAll(`circle.${d}`)
            .style("opacity", !isActive ? 1 : 0.1);

        // Update legend styles globally
